...
import {
  AutocompleteSparqlQueryBuilderIfc,
  ListSparqlQueryBuilderIfc,
  TreeSparqlQueryBuilderIfc,
} from "./SparqlBuilders";
...
export class NoOpListDataProvider implements ListDataProviderIfc {
  ...
  getListContent(
    ...
    // does nothing !
    console.warn(
      "Warning, a NoOpListDataProvider is being called for predicate " +
        predicate
    );
  }
}
      ...
      (data: { results: { bindings: any } }) => {
        ...
        for (let index = 0; index < data.results.bindings.length; index++) {
          ...
          if (solution.uri) {
            ...
            if (columnName) {
              result[result.length] = {
                term: solution[columnName],
                label: solution.label.value,
                group: solution.group?.value,
              };
            } else {
              throw Error(
                "Could not determine which column to read from the result set"
              );
            }
          }
        }
...
export class NoOpAutocompleteProvider implements AutocompleteDataProviderIfc {
  ...
  getAutocompleteSuggestions(
    ...
    // does nothing !
    console.warn(
      "Warning, a NoOpAutocompleteProvider is being called for predicate " +
        predicate
    );
  }
}
...
export class NoOpTreeDataProvider implements TreeDataProviderIfc {
  ...
  getRoots(
    ...
    // does nothing !
    console.warn(
      "Warning, a NoOpTreeDataProvider is being called for predicate " +
        predicate
    );
  }
    ...
    return (data) => {
      ...
      for (let index = 0; index < data.results.bindings.length; index++) {
        ...
        result[result.length] = {
          ...
          hasChildren: solution.hasChildren
            ? solution.hasChildren.value === "true" ||
              solution.hasChildren.value == 1
              ? true
              : false
            : true,
