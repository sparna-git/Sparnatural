...
export default function initGeneralevent(actionStore: ActionStore){
    ...
    actionStore.sparnatural.BgWrapper.componentsList.rootGroupWrapper.traverse((grpWrapper:GroupWrapper)=>{
        previousHeight = currentHeight
        currentHeight = grpWrapper.html.outerHeight(true) + 1
        cssdef += drawBackgroungOfGroupWrapper(index,previousHeight,currentHeight)
        index++
        if(grpWrapper.andSibling) drawLinkAndBottom(grpWrapper,previousHeight,currentHeight)
    })
...
function drawLinkAndBottom(grpWarpper:GroupWrapper,prevHeight:number,currHeight:number){
...
function drawBackgroungOfGroupWrapper(index:number,prev:number,currHeight:number) {
