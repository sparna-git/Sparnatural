...
function buildSparnatural(actionStore:ActionStore,branches:Array<Branch>){
    ...
    let rootGrpWrapper =  actionStore.sparnatural.BgWrapper.componentsList.rootGroupWrapper
...
function buildCriteriaGroup(grpWarpper:GroupWrapper,branch:Branch){
    ...
    if(!(grpWarpper.CriteriaGroup.StartClassGroup.startClassVal.type)){
        ...
        setSelectedValue(grpWarpper.CriteriaGroup.StartClassGroup,startClassVal.type)
    }
    ...
    setSelectedValue(grpWarpper.CriteriaGroup.ObjectPropertyGroup,objectPropVal.type)

    // set WidgetValues
    branch.line.values.forEach(v=>{
        grpWarpper.CriteriaGroup.EndClassGroup.endClassWidgetGroup.renderWidgetVal(v)
    })
    ...
    if(branch.children.length > 0){
        clickOn(grpWarpper.CriteriaGroup.EndClassGroup.editComponents.actionWhere.btn)
...
function setSelectedValue(component:StartClassGroup | EndClassGroup | ObjectPropertyGroup,value:string){
