...
export class QuerySPARQLWriter {
  constructor(
    ...
    var query = parser.parse(
      "PREFIX ex: <http://exemple.fr/> PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>  SELECT ?this WHERE { { ?this ex:date ?date . FILTER('1575-01-01'^^xsd:date < ?date && ?date < '1580-12-31'^^xsd:date) } UNION { ?this ex:beginDate ?beginDate .  FILTER(?beginDate < '1580-12-31'^^xsd:date) ?this ex:endDate ?endDate . FILTER(?endDate > '1575-01-01'^^xsd:date) } }"
    );
  ...
  toSPARQL(q) {
    ...
    if (query.order) {
      sparqlQuery.order = this._initOrder(
        query.order.expression,
        query.order.sort ? query.order.sort : null
      );
    }
  ...
  _preProcessQueryForLabelProperty(jsonQuery) {
    for (var i = 0; i < jsonQuery.branches.length; i++) {
      ...
      jsonQuery.branches[i] = this._preprocessBranchForLabelProperty_rec(
        branch,
        i == 0,
        jsonQuery
      );
  ...
  _preprocessBranchForLabelProperty_rec(
    ...
    for (var i = 0; i < branch.children.length; i++) {
      ...
      branch.children[i] = this._preprocessBranchForLabelProperty_rec(
        child,
        false,
        jsonQuery
      );
