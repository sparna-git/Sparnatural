...
class VariableSelector extends HTMLComponent {
  ...
  constructor(
    ...
    this.globalVariablesSelctor =
      this.GrandParent.thisForm_.sparnatural.variablesSelector;
    this.icon = specProvider.getIcon(ParentComponent.value_selected);
    this.highlightedIcon = specProvider.getHighlightedIcon(
      ParentComponent.value_selected
    );
    this.varLabel = localName(ParentComponent.value_selected);
    this.displayVariableList =
      this.GrandParent.thisForm_.queryOptions.displayVariableList;
  ...
  init() {
    ...
    this.element = $(
      '<div class="sortableItem"><div class="variableSelected flexWrap" data-variableName="' +
        varName +
        '" data-variableLabel="' +
        this.varLabel +
    ...
    this.contentEditableElement.on(
      "focus",
      { arg1: this, arg2: "onFocusEdit" },
      eventProxiCriteria
    );
    this.contentEditableElement.on(
      "focusout",
      { arg1: this, arg2: "onFocusOutEdit" },
      eventProxiCriteria
    );
    ...
    if (
      this.GrandParent.thisForm_.queryOptions.displayVariableList.length == 1
    ) {
      var width = $(".sortableItem").first().width();
      $(".variablesOrdersSelect").width(width);
    }
  ...
  onFocusOutEdit() {
    ...
    if (this.currentValue !== newValue) {
      ...
      $(this.contentEditableElement)
        .parents(".variableSelected")
        .attr("data-variableName", "?" + this.currentValue);
      ...
      if (this.ParentComponent instanceof StartClassGroup) {
        $(this.ParentComponent.html)
          .parents("ul")
          .first()
          .find(">li>.CriteriaGroup>.StartClassGroup")
          .each(function (index) {
            if (index != 0) {
              childs_index[index] = $(this)
                .parents("li")
      ...
      childs_index.forEach((child) => {
        for (var componentKey in this.GrandParent.thisForm_.sparnatural
          .components) {
          if (
            this.GrandParent.thisForm_.sparnatural.components[componentKey]
              .index == child
          ) {
            this.GrandParent.thisForm_.sparnatural.components[
              componentKey
            ].CriteriaGroup.StartClassGroup.varName = "?" + this.currentValue;
          }
        }
      });
  ...
  remove() {
    ...
    this.GrandParent.thisForm_.queryOptions.displayVariableList =
      this.GrandParent.thisForm_.queryOptions.displayVariableList.filter(
        function (value: any, index: any, arr: any) {
    ...
    if (
      this.GrandParent.thisForm_.queryOptions.displayVariableList.length == 0
    ) {
      $(this.GrandParent.thisForm_.sparnatural)
        .find(".selectViewVariable")
        .first()
        .trigger("click");
    }
