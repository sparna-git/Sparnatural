...
class OptionTypeId extends HTMLComponent {
  ...
  constructor(ParentComponent: HTMLComponent, specProvider: ISpecProvider) {
    super("OptionTypeId", ParentComponent, specProvider, null);
...
class OptionSelectBuilder {
  ...
  buildOptionSelect(
    ...
    if (this.specProvider.isEnablingOptional(objectId)) {
      items.optional =
        this.OptionTypeId.GrandParent.thisForm_.langSearch.labelOptionOptional;
    }

    if (this.specProvider.isEnablingNegation(objectId)) {
      items.notExists =
        this.OptionTypeId.GrandParent.thisForm_.langSearch.labelOptionNotExists;
    }
    ...
    for (const [key, value] of Object.entries(items)) {
      ...
      list.push(
        '<label class="flexWrap"><input type="radio" name="' +
          inputID +
          '" data-id="' +
          key +
          '"' +
          ...
          "</div><span>" +
          label +
          '</span><div class="componentFrontArrow">' +
