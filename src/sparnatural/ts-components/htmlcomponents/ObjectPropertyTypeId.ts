...
class ObjectPropertyTypeId extends HTMLComponent {
	...
	constructor(ParentComponent:HTMLComponent, specProvider:ISpecProvider,temporaryLabel:string) {
		super(
	...
	*/
	render() {
		...
		if(this.GrandParent.EndClassGroup.value_selected) {
			...
			this.widgetHtml = $('<span class="current temporary-label">'+this.temporaryLabel+'</span>') ;
			this.init()
		}
	} ;	
