_[Home](../index.html) > Archives > Query JSON format (v12)_

# Sparnatural Query JSON format (v12)

**/!\ This is the query format of Sparnatural up to version 12. Sparnatural now emits and loads the [v13 query format](../Query-JSON-format.md), which is structured very differently (`where` / `predicateObjectPairs` instead of `branches` / `line`).**

> This page is still current for **Sparnatural form** : the file passed in the [`query` attribute](../sparnatural-form/form-integration.md) of `<sparnatural-form>` follows the v12 structure described below. Do not confuse it with the [form query JSON](../sparnatural-form/form-query.md), the flat structure used to _pre-fill_ the form fields.
...
- The _labels_ of the values
- The _types_ of all subjects and objects
- The _tree-like structure_ that Sparnatural uses, which would be hard to guess by looking at the SPARQL string
